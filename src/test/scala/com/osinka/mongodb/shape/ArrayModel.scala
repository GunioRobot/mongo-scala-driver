...
trait ArrayOfEmbeddedTrait {
    class ArrayModel(val id: Int, val users: List[CaseUser])

    trait ArrayModelIn[T] extends ObjectIn[ArrayModel, T] {
      lazy val id = Field.scalar("id", _.id)

      object users extends ArrayEmbeddedField[CaseUser]("users", _.users, None) with CaseUserIn[T]

      lazy val * = List(id, users)
      override def factory(dbo: DBObject) = for {_id <- id.from(dbo); users(_users) <- Some(dbo)} yield new ArrayModel(_id, _users.toList)
    }
}

object ArrayOfEmbedded extends ArrayOfEmbeddedTrait {
    object ArrayModel extends ObjectShape[ArrayModel] with ArrayModelIn[ArrayModel]
}